...
export default class Backtest {
  ...
  drawPriceChart(x, y, width, height) {
    ...
    if (this.showSignals) {
     ...
     // Title (Preço + % mudança + RSI)
  ctx.fillStyle = "#1f2937";
  ctx.font = "bold 14px sans-serif";
  ctx.textAlign = "left";

  if (this.candles && this.candles.length > 0) {
    const currentIndexSafe = Math.min(this.currentIndex, this.endIndex - 1);
    const currentCandle = this.candles[currentIndexSafe];

    if (currentCandle && typeof currentCandle.c === "number") {
      // Preço atual
      ctx.fillText(`${currentCandle.c.toFixed(2)}`, x + padding, y + 12);

      // Variação em %
      if (typeof currentCandle.o === "number" && currentCandle.o !== 0) {
    ...
    if (this.indicators.rsi) {
      const idx = this.currentIndex - this.startIndex;
      if (idx >= 0 && idx < this.indicators.rsi.length) {
        const currentRSI = this.indicators.rsi[idx];
        if (typeof currentRSI === "number") {
          ctx.fillStyle =
            currentRSI > 70 ? "#ef4444" : currentRSI < 30 ? "#10b981" : "#666";
          ctx.font = "11px monospace";
...
drawEquityChart(x, y, width, height) {
  ...
  const scaleX = (i) =>
    x + padding + (i / (history.length - 1)) * (width - 2 * padding - 60);
  const scaleY = (equity) =>
    y + padding + (1 - (equity - minEquity) / range) * (height - 2 * padding);
  ...
  // Performance
  const currentReturn =
    ((this.equity - this.initialBalance) / this.initialBalance) * 100;
  ctx.fillStyle = currentReturn >= 0 ? "#10b981" : "#ef4444";
  ctx.font = "11px monospace";
  ctx.fillText(
    `Return: ${currentReturn >= 0 ? "+" : ""}${currentReturn.toFixed(1)}%`,
    x + padding + 60,
    y + 12
  );
...
onComplete() {
  ...
  alert(
    `📊 Backtest Completo!\n\nRetorno: ${
      ...
      this.totalTrades
    }\nWin Rate: ${winRate.toFixed(1)}%\nMax Drawdown: -${this.maxDrawdown.toFixed(
      1
    )}%`
  );
}
