// OAlgoEngine - núcleo do algoritmo
export default class OAlgoEngine {
  ...
  load(json) {
    if (!json || !json.steps) throw new Error("Invalid algorithm JSON");
    this.algorithm = json;
  ...
  getJSON() { return JSON.stringify(this.algorithm, null, 2); }

  run(context = {}) {
    this.algorithm.steps.forEach((s) => {
